...
def classify_exp(title):
    ...
    if any(k in t for k in ["junior", "jr", "associate", "entry level", "graduate"]): return "junior"
...
def classify_job_type(title, desc):
    ...
    if any(k in text for k in ["intern", "internship", "trainee"]): return "internship"
    if any(k in text for k in ["freelance", "freelancer"]): return "freelance"
    if any(k in text for k in ["contract", "contractor", "c2c"]): return "contract"
    if any(k in text for k in ["part-time", "part time"]): return "part-time"
    return "full-time"
...
async def verify_and_insert(raw_jobs):
    ...
    print(f"  Summary -> Inserted: {inserted} | Invalid: {skipped_invalid} | Duplicates: {skipped_dup}")
    return inserted
...
async def main():
    ...
    # --- INDIA-FOCUSED QUERIES (simple, proven to return 1000s of results) ---
    india_queries = [
        ...
        "fresher software engineer", "remote backend developer", "internship data science",
