...
export default function FilterPanel({ show, onClose }) {
      ...
      {isMobile && (
        <div style={{ paddingTop: 12, paddingBottom: 20 }}>
          <button className="btn btn-primary" onClick={onClose}
            style={{ width: '100%', borderRadius: 12, fontSize: 15, padding: '14px 24px' }}>
            Show Results
          </button>
        </div>
      )}
  ...
  // ─── MOBILE: fixed full-screen drawer ────────────
  if (isMobile) {
    return (
      <AnimatePresence>
        {show && (
          <>
            ...
            <motion.aside
              ...
              style={{
                ...
                width: '85vw', maxWidth: 340, zIndex: 56,
  ...
  // ─── DESKTOP: original inline sidebar (unchanged) ────────
  return (
    <AnimatePresence>
      {show && (
        <motion.aside
